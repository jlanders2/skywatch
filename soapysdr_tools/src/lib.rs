...
impl Sdr for SoapySdr {
    fn create_device(&self) -> Result<Box<dyn SdrDevice>, SdrError> {
        ...
        let selected_device_idx: usize = device_input.trim().parse::<usize>().expect("Input is a number");
...
impl SdrDevice for SoapySdrDevice {
    ...
    fn set_sample_rate(&mut self, sample_rate: f64) -> Result<(), SdrError> {
        self.device.set_sample_rate(self.direction, self.channel, sample_rate).expect("Successfully set sample rate for device");

        return Ok(());
    }

    fn set_frequency(&mut self, frequency: f64) -> Result<(), SdrError> {
        self.device.set_frequency(self.direction, self.channel, frequency, "").expect("Successfully set frequency for device");

        return Ok(());
    }

    fn set_gain(&mut self, gain: f64) -> Result<(), SdrError> {
        self.device.set_gain(self.direction, self.channel, gain).expect("Successfully set gain for device");

        return Ok(());
    }

    fn get_stream(&self) -> Result<Box<dyn SdrStream>, SdrError> {
        let mut rx_stream = self.device.rx_stream::<Complex<f32>>(&[self.channel]).expect("Successfully created Rx stream for device");
...
impl SdrStream for SoapySdrStream {
    fn read(&mut self) -> Result<Vec<f32>, SdrError> {
        ...
        let scalars: Vec<f32> = self.buffer[..samples_read].iter().map(|s|s.norm()).collect();
